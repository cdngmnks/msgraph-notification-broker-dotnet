...
namespace GraphSubscriber.Services
{
    public class RedisFactory : IRedisFactory
    {
        ...
        private static Lazy<List<RedisKey>> _keys;
        ...
        public RedisFactory(ILogger<RedisFactory> logger)
        {
            _logger = logger;
        ...
        public List<RedisKey> GetServerKeys()
        {
            if (_disposed) throw new ObjectDisposedException(nameof(RedisFactory));
        ...
        private Lazy<IConnectionMultiplexer> CreateMultiplexer()
        {
            ...
            return new Lazy<IConnectionMultiplexer>(() => ConnectionMultiplexer.Connect(Environment.GetEnvironmentVariable("RedisConnectionString")));
        }
        ...
        private Lazy<List<RedisKey>> GetAllKeys()
        {
            return new Lazy<List<RedisKey>>((List<RedisKey>)_multiplexer.Value.GetServer(_multiplexer.Value.GetEndPoints().FirstOrDefault()).Keys().ToList());
        }
